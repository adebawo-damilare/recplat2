...
The default **`quality`** job still runs **`npm run test:e2e`** **without** **`E2E_RUN_AUTH`** (public / unauthenticated browser tests only; **`e2e/authenticated/**`** is ignored). To run authenticated E2E locally with Postgres in **`.env.local`**, use **`npm run test:e2e:auth`** (sets **`E2E_RUN_AUTH=1`**). Playwright **starts its own dev server** with E2E env (admin allowlist, fake email ledger) — do not rely on an already-running `npm run dev` unless you copy the **`TALENTBRIDGE_E2E_*`** vars from **`.env.example`** into **`.env.local`**. Prod email/admin: **`docs/PROD_EMAIL_ADMIN.md`**.
