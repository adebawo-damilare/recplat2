...
import { Search, Mail, User, Briefcase, FileText, Code, Database, X, ExternalLink } from "lucide-react";
import { getAllCandidates, seedCandidates, auth, type CandidateProfile } from '../lib/firebase';
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      if (updated) {
        ...
        // Refresh selected candidate if it exists
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-10">
        ...
        {!loading && (
          <button 
            ...
            title="Update sample candidates with latest data"
          >
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-neutral-100 flex items-center justify-center overflow-hidden">
                    <img src={`https://picsum.photos/seed/${c.userId}/100/100`} alt={c.fullName} className="w-full h-full object-cover" />
                  </div>
                ...
                {c.skills.split(',').slice(0, 3).map((skill, sIdx) => {
                  ...
                  return (
                    <span key={`${c.userId}-skill-${sIdx}`} className="px-1.5 py-0.5 bg-neutral-100 text-neutral-500 rounded text-[10px] font-bold uppercase truncate">
                      {trimmed}
                    </span>
                  );
                })}
                  {c.skills.split(',').length > 3 && <span className="text-[10px] text-neutral-400 font-bold">+{c.skills.split(',').length - 3}</span>}
                </div>
              </motion.div>
            ))
          ) : (
        ...
        <div className="lg:col-span-2">
          {selectedCandidate ? (
            ...
            <div className="h-full min-h-[500px] flex flex-col items-center justify-center bg-neutral-50 rounded-3xl border-2 border-dashed border-neutral-200 p-8 text-center">
              ...
              <p className="text-neutral-500 max-w-sm">Browse full resumes, portfolios, and contact information to find your ideal team member.</p>
            </div>
          )}
        </div>
      </div>
