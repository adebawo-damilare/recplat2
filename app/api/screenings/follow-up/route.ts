...
import { listRecruiterScreeningFollowUpForOwner } from "../../../../src/server/screenings";
import { isScreeningEnabledCategorySlug } from "../../../../src/shared/screeningPilot";
import { getClientKey } from "../../../../src/server/rateLimit";

export async function GET(request: NextRequest) {
  const key = getClientKey(
    request.headers.get("x-forwarded-for") || request.headers.get("x-real-ip"),
    "/api/screenings/follow-up",
  );
  ...
  const categorySlug = categorySlugParam?.trim().toLowerCase();
  const items = await listRecruiterScreeningFollowUpForOwner(authResult.user.userId, {
    categorySlug:
      categorySlug && isScreeningEnabledCategorySlug(categorySlug) ? categorySlug : undefined,
  });
